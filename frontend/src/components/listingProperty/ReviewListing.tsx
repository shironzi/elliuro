    ...
    async function fetchProperty(): Promise<void> {
      if (propertyId) {
        try {
          ...
          const amenities: { name: string; value: number }[] =
    ...
    <div>
      <div className="bg-darkGray-400">
        <Details
          ...
          bedroom={
            amenities.find((amenity) => amenity.name === 'bedroom')?.value || 0
          }
          toilet={
            amenities.find((amenity) => amenity.name === 'bathroom')?.value || 0
          }
          SwimmingPool={
            amenities.find((amenity) => amenity.name === 'swimmingPool')
              ?.value || 0
          }
          carPort={
            amenities.find((amenity) => amenity.name === 'carPort')?.value || 0
          }
        />
      </div>
      <button className="fixed bottom-10 left-1/2 transform -translate-x-1/2 bg-slate-900 text-white py-4 px-10 w-fit">
        PUBLISH
      </button>
    </div>
  )
